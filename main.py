...
def cansel(message):
    bot.send_message(message.chat.id, "Чтобы посмотреть команды, используй - /info", reply_markup=hideBoard) # Это кнопка отмены, но есть не везде
  
def no_projects(message):
    bot.send_message(message.chat.id, 'У тебя пока нет проектов!\nМожешь добавить их с помошью команды /new_project') # Если нет проекта, когда пишешь команду /projects, то пишется сообщение в чате "У тебя пока нет проектов! Можешь добавить их с помошью команды /new_project"
...
def gen_inline_markup(rows):
    ...
    return markup # Это функция для работы Inline-клавиатуры
...
def gen_markup(rows):
    ...
    return markup # А это функция для работы Reply-клавиатуры
...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, "Введите ссылку на проект (если нету ссылки на проект, то вводите null)")
